...
unsigned long int F(const BF_KEY *key, BF_LONG X){
	uint8_t a, b, c, d;
	a = (X >> 24)&0xFF;
	b = (X >> 16)&0xFF;
	c = (X >> 8)&0xFF;
	...
	return (key->S[ S_INDEX(0, a) ] + key->S[ S_INDEX(1, b)] ^ key->S[ S_INDEX(2, c)]) + key->S[S_INDEX(3, d)];
}
...
void BF_set_key(BF_KEY *key, int len, const unsigned char *data){
	...
	for(int i = 0, b = 0; i < P_SIZE; ++i, b += 4){
		key->P[i] = P_ARRAY[i];
		key->P[i] ^= ((BF_LONG)data[(b)   % len]) << 24;
		key->P[i] ^= ((BF_LONG)data[(b+1) % len]) << 16; 
		key->P[i] ^= ((BF_LONG)data[(b+2) % len]) << 8 ;
		key->P[i] ^= ((BF_LONG)data[(b+3) % len])      ;
	}
 ...
 *enc decides if encryption (BF_ENCRYPT) or decryption (BF_DECRYPT) shall be performed. 
...
void *fs_encrypt(void *plaintext, int bufsize, char *keystr, 
	...
	size = bufsize;
	BF_set_key(key, key_len, (const unsigned char *)keystr);
	BF_ecb_encrypt(text, (unsigned char *)out, key, BF_ENCRYPT);
	*resultlen = size / 2;
...
void *fs_decrypt(void *ciphertext, int bufsize, char *keystr, 
	...
	size = bufsize;
	BF_set_key(key, key_len, (const unsigned char *)keystr);
	BF_ecb_encrypt(text, (unsigned char *)out, key, BF_DECRYPT);
	*resultlen = size;
